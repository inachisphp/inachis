...
class ImportDetector
{
    ...
    public function detectImportType(array $data): string
    {
        ...
        return match (true) {
            ...
            array_key_exists('parentId', $firstItem) || array_key_exists('fullPath', $firstItem) => 'category',

            // Post: Unique key 'content' (contains the long-form markdown/HTML)
            array_key_exists('content', $firstItem) => 'post',

            // Series: Unique keys 'items' (the list of post titles) or 'firstDate'
            array_key_exists('items', $firstItem) && array_key_exists('url', $firstItem) => 'series',
